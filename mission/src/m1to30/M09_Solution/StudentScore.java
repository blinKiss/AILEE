...
public class StudentScore {
	...
	public static void calcRank(int classNo) {
		...
		// 반 등수 계산
		for (int i = 0; i < classStudentRank[classNo-1].length; i++) {
			classStudentRank[classNo-1][i][0] = 1;
		}
		...
		for (int i = 0; i < classStudentData[classNo-1].length; i++) {
			for (int j = 0; j < classStudentData[classNo-1].length; j++) {
				...
				if( classStudentData[classNo-1][i][0] < classStudentData[classNo-1][j][0] ) {
					classStudentRank[classNo-1][i][0]++;
				} 
		...
		for (int x = 0; x < 6; x++) {
			...
			for (int i = 0; i < subjectStudentRank[classNo-1].length; i++) {
				subjectStudentRank[classNo-1][i][x][0] = 1;
			}
			
			for (int i = 0; i < classStudentScore[classNo-1].length; i++) {
				for (int j = 0; j < classStudentScore[classNo-1].length; j++) {
					
					if( classStudentScore[classNo-1][i][x] < classStudentScore[classNo-1][j][x] ) {
						subjectStudentRank[classNo-1][i][x][0]++;
					} 
		...
		for (int x = 0; x < 6; x++) {
			...
			int [][] allStudentSubjectRank = new int[allStudentCount][2];
			
			for (int i = 0; i < allStudentSubjectRank.length; i++) {
				allStudentSubjectRank[i][1] = 1;
			}
			...
			for (int i = 0; i < classStudentScore.length; i++) {
				...
				for (int j = 0; j < classStudentScore[i].length; j++) {
					if( !studentYn[i][j] ) continue;
					allStudentSubjectRank[k++][0] = (int) classStudentScore[i][j][x];
				}
			}
			
			for (int i = 0; i < allStudentSubjectRank.length; i++) {
				for (int j = 0; j < allStudentSubjectRank.length; j++) {
					if( allStudentSubjectRank[i][0] < allStudentSubjectRank[j][0] )
						allStudentSubjectRank[i][1]++;
				}
			}
			...
			for (int i = 0; i < classStudentRank.length; i++) {
				...
				for (int j = 0; j < classStudentRank[i].length; j++) {
					if( !studentYn[i][j] ) continue;
					subjectStudentRank[i][j][x][1] = allStudentSubjectRank[k++][1];
				}
			}
		}
		// 과목별 전교 등수 계산 (끝)
	...
	public static void calcClassTotal(int classNo) {
		...
		// 과목별 평균
		for (int j = 0; j < classTotalScore[classNo-1].length; j++) {
			if( classTotalScore[classNo-1][j] == 0 ) continue;
			classTotalAverage[classNo-1][j] = Math.round( (double)classTotalScore[classNo-1][j] / classCount[classNo-1] * 100.0 ) / 100.0;
		}
