package m1to30.M12_copy;

import java.util.Scanner;

import m1to30.M12.Account_cv;

public class Bank2 {
	private static Account_cv[] accountArray = new Account_cv[1000];
	...
	private static void createAccount() {
		...
		for (int i = 0; i < accountArray.length; i++) {
			if (accountArray[i] == null) {
				accountArray[i] = new Account_cv(ano, owner, amount);
	...
	// 입력된 계좌번호로 계좌정보 조회
	private static Account_cv findAccount(String ano) {
		Account_cv account = null;
		for (int i = 0; i < accountArray.length; i++) {
			if (accountArray[i] != null)
