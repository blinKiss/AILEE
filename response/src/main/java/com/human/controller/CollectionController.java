...
/**
 * 컬렉션 타입 응답
 * - 반환 타입을 컬렉션 타입으로 하면, JSON 객체 배열 형식으로 변환되어 응답한다
 * @author denim
 *
 */
@Slf4j
...
public class CollectionController {
	...
	public List<Member> memberList(){
		...
		for (int i = 0; i < 5; i++) {
			Member member = new Member(i+1, "human", "123456", "김휴먼"
				       ,"khm@human.com", new Date(), new Date());
	...
	public Map<String, Member> memberMap( ) {
		...
		for (int i = 0; i < 5; i++) {
			memberMap.put("회원" + (i+1), new Member(i+1, "human", "123456", "김휴먼"
				       ,"khm@human.com", new Date(), new Date()));
		}
