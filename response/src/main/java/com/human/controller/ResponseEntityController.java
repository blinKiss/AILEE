...
/**
 * ResponseEntity<> 응답 객체 - 응답 메시지의 헤더정보와 내용을 설정하는 객체 - 생성 : new
 * ResponseEntity<응답할 데이터 타입>(객체, 상태코드); * HttpStatus.상태코드상수
 * 
 * - ex) new ResponseEntity<String>("OK", HttpStatus.OK); - ex) new
 * ResponseEntity<Board>(new Board(), HttpStatus.OK);
 * 
 * @author denim
 *
 */
...
public class ResponseEntityController {
	...
	public ResponseEntity<Map<String, Member>> memberMap() {
		...
		for (int i = 0; i < 5; i++) {
			memberMap.put("회원" + (i + 1),
					new Member(i + 1, "human", "123456", "김휴먼", "khm@human.com", new Date(), new Date()));
		}
	...
	public ResponseEntity<byte[]> responseImg() throws Exception {
		...
		try {
			// 집
			File file = new File("C:\\god\\upload\\test.jpg");
			// File --> byte[]
	...
	public ResponseEntity<byte[]> downloadImg() throws Exception {
		...
		try {
			...
			fileName = new String(fileName.getBytes("UTF-8"), "ISO-8859-1"); // 인코딩된 파일
			
			// Content-Disposition : attatchment; filename="파일명.확장자"
